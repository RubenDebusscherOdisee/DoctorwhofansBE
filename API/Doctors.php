...
header('Content-Type: application/json');
	...
	$antwoord['Doctors'] = "Geen resultaten gevonden.";
    //stel the charset in
	mysqli_set_charset($conn,'utf8');
	/* prepare de query (maak de query zonder de variabelen op te nemen)*/
	$stmt1 = $conn->prepare("select * from doctors inner join actors on doctors.primary_actor=actors.id order by volgorde asc");
	...
	if(!$stmt1->execute()){
        ...
        $antwoord['Doctors'] = $result->fetch_all(MYSQLI_ASSOC);
