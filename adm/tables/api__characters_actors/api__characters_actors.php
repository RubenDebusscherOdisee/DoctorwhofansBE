<?php
class tables_api__characters_actors {
  
  function beforeInsert(Dataface_Record $record){
    $user = Dataface_AuthenticationTool::getInstance()->getLoggedInUser();
    if ( $user and !$record->val('AC_Owner_Id') ){
        $record->setValue('AC_Owner_Id', $user->val('user_Id'));
        $record->setValue('AC_Last_modifier', $user->val('user_Id'));
  ...
  function __import__csv($data, $defaultValues=array()){
    ...
    $rows = explode("\n", $data);
    foreach ( $rows as $row ){
      list($actor_Id,$Character_Id,$Character_Type) = explode(',', $row);
      $record = new Dataface_Record('api__characters_actors', array());
      $record->setValues($defaultValues);

      $record->setValues(
        array(
          'AC_actor_Id'=>$actor_Id,
          'AC_Character_Id'=>$Character_Id,
          'AC_Type'=>$Character_Type
        )
      );
      $records[] = $record;
    }
  ...
  function __import__excel_spreadsheet($data, $defaultValues=array()){
    ...
    while ($objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)!=""){
      ...
      $record->setValues(
        array(
          'AC_actor_Id'=>$objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)->getValue(),
          'AC_Character_Id'=>$objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1, $ligne)->getValue(),
          'AC_Type'=>$objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2, $ligne)->getValue()
          )
      );
      $records[] =$record;
      $ligne++;
