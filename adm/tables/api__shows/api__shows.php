<?php
class tables_api__shows {
  function getTitle($record){
		return $record->val('show_Id').': '.$record->val('show_Name');
    }
  function beforeInsert(Dataface_Record $record){
    $user = Dataface_AuthenticationTool::getInstance()->getLoggedInUser();
    if ( $user and !$record->val('show_Owner_Id') ){
        $record->setValue('show_Owner_Id', $user->val('user_Id'));
        $record->setValue('show_Last_modifier', $user->val('user_Id'));
  ...
  function __import__csv($data, $defaultValues=array()){
    ...
    $rows = explode("\n", $data);
    foreach ( $rows as $row ){
      list($show_Name) = explode(',', $row);
      
      $record = new Dataface_Record('api__shows', array());            
  ...
  function __import__excel_spreadsheet($data, $defaultValues=array()){
    ...
    while ($objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)!=""){
      ...
      $record->setValues(
        array(
          'show_Name'=>$objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)->getValue(),
          )
      );
      $records[] =$record;
      $ligne++;
