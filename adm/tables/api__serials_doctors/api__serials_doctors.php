<?php
class tables_api__serials_doctors {
  
  function beforeInsert(Dataface_Record $record){
    $user = Dataface_AuthenticationTool::getInstance()->getLoggedInUser();
    if ( $user and !$record->val('serials_doctors_Owner_Id') ){
        $record->setValue('serials_doctors_Owner_Id', $user->val('user_Id'));
        $record->setValue('serials_doctors_Last_modifier', $user->val('user_Id'));
  ...
  function __import__csv($data, $defaultValues=array()){
    ...
    foreach ( $rows as $row ){
      ...
      $record = new Dataface_Record('api__serials_doctors', array());
      $record->setValues($defaultValues);

      $record->setValues(
        array(
          'DF_Doctor_Id'=>$doctor_Id,
          'DF_Serial_Id'=>$serial_Id
        )
      );
      $records[] = $record;
    }
  ...
  function __import__excel_spreadsheet($data, $defaultValues=array()){
    ...
    while ($objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)!=""){
      ...
      $record->setValues(
        array(
          'DF_Doctor_Id'=>$objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)->getValue(),
          'DF_Serial_Id'=>$objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1, $ligne)->getValue(),
