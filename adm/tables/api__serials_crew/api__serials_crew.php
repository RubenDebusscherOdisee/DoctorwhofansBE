<?php
class tables_api__serials_crew {
  function beforeInsert(Dataface_Record $record){
    $user = Dataface_AuthenticationTool::getInstance()->getLoggedInUser();
    if ( $user and !$record->val('serials_crew_Owner_Id') ){
        $record->setValue('serials_crew_Owner_Id', $user->val('user_Id'));
        $record->setValue('serials_crew_Last_modifier', $user->val('user_Id'));
  ...
  function __import__csv($data, $defaultValues=array()){
    ...
    foreach ( $rows as $row ){
      ...
      $record = new Dataface_Record('api__serials_crew', array());
      $record->setValues($defaultValues);

      $record->setValues(
        array(
          'SC_Crew_Id'=>$doctor_Id,
  ...
  function __import__excel_spreadsheet($data, $defaultValues=array()){
    ...
    while ($objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)!=""){
      ...
      $record->setValues(
        array(
          'SC_Crew_Id'=>$objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0, $ligne)->getValue(),
