<?php
	...
	$stmt1 = $conn->prepare("SELECT concat(Topics.id,' - ',Topics.topic,' (',Topics.link,')') as pagina,COUNT(A_id) as totaal,SUM(if(A_Actief = 1, 1, 0)) as Actief,SUM(if(A_Actief = 0, 1, 0))as dormant FROM alles left outer join Topics on alles.A_Pagina=Topics.id group by A_Pagina UNION SELECT concat(Topics.topic,' (',Topics.link,')') as pagina,COUNT(A_id) as totaal, SUM(if(A_Actief = 1, 1, 0)) as Actief,SUM(if(A_Actief = 0, 1, 0))as dormant FROM alles right outer join Topics on alles.A_Pagina=Topics.id group by A_Pagina");
