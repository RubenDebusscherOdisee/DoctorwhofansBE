<?php
	...
	$stmt1 = $conn->prepare("SELECT dayname(CONVERT_TZ(A_TIMESTAMP,'UTC','Europe/Brussels')) as day, concat(lpad(hour(CONVERT_TZ(A_TIMESTAMP,'UTC','Europe/Brussels')),2,0),' - ',lpad(hour(TIMESTAMPADD(hour,1,CONVERT_TZ(A_TIMESTAMP,'UTC','Europe/Brussels'))),2,0))as uur, count(A_Pagina) aantal_elem  FROM `Content` group by day,uur order by weekday(CONVERT_TZ(A_TIMESTAMP,'UTC','Europe/Brussels')),uur");
