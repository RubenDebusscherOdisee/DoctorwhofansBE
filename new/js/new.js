...
function getAvailableLangcodes() {
	$.ajax({
		type: "GET",
		url: "https://www.doctorwhofans.be/php/getAvailable_Lang.php",
		dataType: 'json',
		crossDomain: true,
		cache: false
	}).done(
...
function GetContent(menu){
  if(menu !=""){
    ...
    var settings = {
      "url": "https://www.doctorwhofans.be/new/php/checkPage.php",
      "method": "POST",
      "timeout": 0,
      "dataType": 'json',
      "processData": false,
...
function Doctor(DoctorData){
  ...
  WikiImage(DoctorData.Doctor[0].doctor_Image,'api__doctors');
  //Info(DoctorData);
