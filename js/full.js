		...
		function (resultaat) {
			...
			for (i = 0; i < resultaat.data.length; i += 1) {
				if (resultaat.data[i].link.indexOf("Forum") >= 0) {
					$("#resultSet").append("<a href='" + resultaat.data[i].link + "'  >" + resultaat.data[i].content + " (Forum)</a>");
				} else {
					$("#resultSet").append("<a href='" + resultaat.data[i].link + "'  >" + resultaat.data[i].content + "</a>");
				}
			}
		}).fail(function (response, statusText, xhr) {}).always(function () {});
}
	...
	$.ajax({
		...
		for (i = 0; i < resultaat.data.length; i += 1) {
			...
			if (resultaat.data[i].A_Type === "Inleiding") {
				...
				$(".col-6").append("<div id='Inhoud' class='bordered DarkBlueBackground'>" + translations[0].TOC + "<ol></ol></div>");
			}
