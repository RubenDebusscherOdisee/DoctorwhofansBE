var aantalrecords;var img;var eerdermenu;var taal;var menu;var status;var currentTopic;var user;var UID=0;var toon = "false";var slideIndex=2;var ip;var session;var CONTENTID;
if (jQuery) {
	(function($) {
...
function ip_callback() {
    $.get("/php/getIp.php",function(data){ ip=data; })
}
...
function messagezoeken(taal, menu, id, parent, child, zoekterm) {
	$.ajax({
		type: "GET",
		url: "/php/zoekenmessage.php?taal=" + taal + "&menu=" + menu + "&zoekterm=" + zoekterm,
		dataType: 'json',cache: false
	}).done(
		function(resultaat) {
			$("#resultset4").append("<div class ='result slide' id='resultmessages'></div>");
			var i;
			for (i = 0; i < resultaat.data.length; i += 1) {
			    if (resultaat.data[i].link.indexOf("Forum") >= 0) {
			        $("#resultmessages").append("<a href='../" + resultaat.data[i].link + "/'  ><h3>" + resultaat.data[i].topic + " (Forum: "+resultaat.data[i].Aantal+")</h3></a>");
			    }
                else {
                    $("#resultmessages").append("<a href='../" + resultaat.data[i].link + "/'  ><h3>" + resultaat.data[i].topic + "</h3></a>");
                }
...
function getpad(menu) {
	$.ajax({
		...
		function(resultaat){
		    ...
		    if (menu==="Forum"|| resultaat.data[0].pad.search("Forum") >0){
                        if(status==="Admin"){
                            $(".col-6").append("<div class='addtopic'></div>");
                            $(".addtopic").append("<form><fieldset>Voeg een topic toe:</fieldset></form>");
                            $(".addtopic form fieldset").append("<input type='text' placeholder='Pagina' id='topicName'/>@");
                            $(".addtopic form fieldset").append("<input type='text' placeholder='link' id='TopicLink'/>");
                            $(".addtopic form fieldset").append("<input type='submit' value='Add Topic' onclick='AddTopic()'/>");
                        }else{
                            $(".col-6").append("<div class='addtopic'></div>");
                            $('.addtopic').text('Je moet een administrator zijn om een topic toe te kunnen voegen');
                        }
                        $(".col-6").append("<div class='topics'></div>");
                        $(".col-6").append("<div class='messages'></div>");
                        $(".col-6").append("<div class='invoer'></div>");
                        getTitel(menu);
                        getchildren(menu);
                        getMessages(menu);
...
function getMessages(menu) {
	$.ajax({
		type: "GET",
		url: "/php/messages.php?menu=" + menu,
		dataType: 'json',
		crossDomain: true,cache: false
	}).done(
		function(resultaat){
		    if (resultaat.data !=="No Rows"){
		        for (i = 0; i < resultaat.data.length; i += 1) {
		            if (resultaat.data[i].Gebruiker===user||status==="Admin"){
		                var message="";
		                message+="<div class='message'><div class='Gebruiker_Message'><img src='../"+ resultaat.data[i].Avatar+"' title='"+ resultaat.data[i].Gebruiker+" alt='"+ resultaat.data[i].Gebruiker+"'/>";
		                message+="<span style='margin-left:0.5em;'>"+ resultaat.data[i].Gebruiker +"</span><br><span style='margin-left:0.5em;'>"+resultaat.data[i].Status+"</span></div><div class='Message_message'>"+ resultaat.data[i].Bericht +"</div>";
		                message+="<div><span>"+resultaat.data[i].Tijd+"</span><span style='float:right'><a href='#' onclick='RemoveMessage("+resultaat.data[i].Bericht_ID+")'>Remove Message</a></span></div></div>";
		                $('.messages').append(message);
		        
		            }else{
		                $('.messages').append("<div class='message'><div class='Gebruiker_Message'><img src='../"+ resultaat.data[i].Avatar+"' title='"+ resultaat.data[i].Gebruiker+" alt='"+ resultaat.data[i].Gebruiker+"'/><span style='margin-left:0.5em;'>"+ resultaat.data[i].Gebruiker +"</span><br><span style='margin-left:0.5em;'>"+resultaat.data[i].Status+"</span></div><div class='Message_message'>"+ resultaat.data[i].Bericht +"</div><div><span>"+resultaat.data[i].Tijd+"</span></div></div>");
		            }
		            
		        }
		    }else{
		        $('.messages').append("Ben jij de eerste die hier een bericht plaatst?");
		    }
...
function getTitel(menu) {
	$.ajax({
		type: "GET",
		url: "/php/Titel.php?menu=" + menu,
		dataType: 'json',
		crossDomain: true,
	}).done(
		function(resultaat){
            var i;
		for (i = 0; i < resultaat.data.length; i += 1) {
		    $('.col-6').prepend('<h1>'+resultaat.data[i].topic+'</h1>');
		    currentTopic = resultaat.data[i].id;
			if(resultaat.data[i].MagEditeren === "Ja"){
			    
		                $(".invoer").append("<label for='mytextarea' class='privaat'>Message:</label>");
		                $(".invoer").append("<input type='submit' name='submit' value='Verzenden' id='SendMessageButton' type='button' onclick='AddMessage()'/><br>");
                        $(".invoer").append("<textarea id='txtEditor'></textarea>");
                        $("#txtEditor").Editor();

                        

		    }else{
		        $(".messages").prepend("Vraag aan een administrator om toegang tot dit Topic open te stellen.");
		    }
		}
	    ...
	    for(i=0;i < resultaat.data.length;i++){
	        if(id===resultaat.data[i].id){
	            ...
	            if(resultaat.data[i].SPOILER===1){
	                 $(".vid_choice").append("<a href='#' onclick=event.preventDefault();$('.col-6').html('');videosophalen('Video',"+resultaat.data[i].id+") class='inline_block_vid' ><div class=keuze><div><img src='../"+resultaat.data[i].Video_Image+"' class='thumbnail' style='display:inline;background-color:red' alt='"+resultaat.data[i].Video_Name+"'></div><p>"+resultaat.data[i].Video_Name.substr(0, 15)+"...</p></div></a>");
	            }else{
	                 $(".vid_choice").append("<a href='#' onclick=event.preventDefault();$('.col-6').html('');videosophalen('Video',"+resultaat.data[i].id+") class='inline_block_vid' ><div class=keuze><div><img src='../"+resultaat.data[i].Video_Image+"' class='thumbnail' style='display:inline;' alt='"+resultaat.data[i].Video_Name+"'></div><p>"+resultaat.data[i].Video_Name.substr(0, 15)+"...</p></div></a>");
	            }
...
function quotesophalen(menu,id) {
	$.ajax({
	    ...
	    $(".col-6").append("<div class='linkerquote'></div>");
	    $(".col-6").append("<div class='rechterquote'></div>");
	    if(id==="0"){
	        id=Math.floor((Math.random()*resultaat.data.length) + 1);
	    }else{id=id;}
	    for(i=0;i < resultaat.data.length;i++){
	        ...
	        if(id===resultaat.data[i].id){
	           $(".main_quote").append("<h1>"+resultaat.data[i].Personage+" - "+resultaat.data[i].Aflevering+"</h1>");
	           $(".main_quote").append("<div><p class='quotetext'>"+resultaat.data[i].Quote+"</p><img class='quote_picture' src='"+resultaat.data[i].QuotePic+"'/><div>");
	        }else{
...
/*function AddTopic(){
    event.preventDefault();
    var topic={};
    topic.ouder= currentTopic;
    topic.name = $('#topicName').val();
    topic.link = $('#TopicLink').val();
    $.ajax({
		type: "POST",
...
function AddMessage(){
    event.preventDefault();
    var message={};
    if(currentTopic===""||UID===""||$("#txtEditor").Editor("getText")===""){
        alert("Gelieve in te loggen en/of een bericht te plaatsen");
    }else{
        message.topic= currentTopic;
        message.message = $("#txtEditor").Editor("getText");
        message.user = UID;
        $.ajax({
    		type: "POST",
...
*/
function Userophalen(taal, menu, User_ID) {
	$.ajax({
		type: "GET",
		url: "/php/User.php?taal=" + taal + "&menu=" + menu + "&User=" + User_ID,
		dataType: 'json',cache:false
	}).done(function(resultaat) {
		var i;
		for (i = 0; i < resultaat.data.length; i += 1) {
		    if(menu==="Profiel"){
		        ...
		        $(".col-6").append("<img src='" + resultaat.data[i].User_Avatar + "' class='avatar' alt='" + resultaat.data[i].User_Name + " title='" + resultaat.data[i].User_Name + "'>");
			    $(".col-6").append("<p class='user_Info'>Familienaam: " + resultaat.data[i].User_Name + "</p>");
			    $(".col-6").append("<p class='user_Info'>Voornaam: " + resultaat.data[i].User_FirstName + "</p>");
    			$(".col-6").append("<p class='user_Info'>Gebruikersnaam: " + resultaat.data[i].User_Username + "</p>");
    			$(".col-6").append("<p class='user_Info'>E-mail: " + resultaat.data[i].User_email + "</p>");
    			$(".col-6").append("<p class='user_Info'>Geboren op: " + resultaat.data[i].User_Geboortedatum + "</p>");
    			$(".col-6").append("<p class='user_Info'>Status: " + resultaat.data[i].User_Status + "</p>");
				if(resultaat.data[i].User_Status==="Admin" ||resultaat.data[i].User_Status==="User"){
		            $(".col-6").append('<form method="post" action="" enctype="multipart/form-data" id="myform"><div ></div><div ><input type="file" id="file" name="file" /><input type="button" class="button" value="Upload" id="but_upload" onclick="Upload()"></div></form>');
    		    }
		    }
...
/*function RemoveMessage(ID){
    var message={};
    message.ID= ID;
    if (confirm("Bent u zeker dat u uw bericht wil verwijderen?")) {
                $.ajax({
		type: "POST",
		url: "/php/RemoveMessage.php",
		dataType: 'json',
		crossDomain: true,
		data:message,cache:false
	}).done(
		function(resultaat){
...
if (jQuery(this).parent().next().attr('display')==='none'||jQuery(this).text()=="+"){
    jQuery(this).text("-");
}else if(jQuery(this).parent().next().attr('display')==='block'||jQuery(this).text()=="-"){jQuery(this).text("+");}
